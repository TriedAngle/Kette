...
impl BigNum {
    ...
    unsafe fn order_by_abs(lhs: *mut BigNum, rhs: *mut BigNum) -> (*mut BigNum, *mut BigNum, bool) {
    ...
    pub unsafe fn add(
        ...
        let mut carry = 0u8;
        for i in 0..max_size {
            let sum = (*bigger).get_digit(i) as u16 + (*smaller).get_digit(i) as u16 + carry as u16;
    ...
    pub unsafe fn sub(
        ...
        for i in 0..result_size {
            let mut diff = (*bigger).get_digit(i) as i16 - (*smaller).get_digit(i) as i16 - borrow;
        ...
        if actual_size != result_size {
            (*result).meta = BigNum::encode_meta(actual_size as u64, (*result).is_negative());
        }
    ...
    pub unsafe fn mul(
        ...
        for i in 0..lhs_size as usize {
            ...
            for j in 0..rhs_size as usize {
                let prod = (*lhs).get_digit(i) as u16 * (*rhs).get_digit(j) as u16
        ...
        if actual_size != result_size {
            (*result).meta = BigNum::encode_meta(actual_size as u64, result_neg);
        }
    ...
    pub unsafe fn div(
        ...
        Self::copy_digits((*lhs).digits(), (*remainder).digits(), lhs_size as usize);

        for i in (0..=((lhs_size - rhs_size) as usize)).rev() {
            let mut q = ((*remainder).get_digit(i + rhs_size as usize - 1) as u16 * 256
    ...
    pub unsafe fn modulo(
                    ...
                    < (*rhs).get_digit(rhs_size as usize - 1))
        {
            ...
            Self::copy_digits((*lhs).digits(), (*result).digits(), lhs_size as usize);
            return result;
        }
        ...
        if actual_size != lhs_size as usize {
            (*remainder).meta = BigNum::encode_meta(actual_size as u64, lhs_neg);
        }
    ...
    pub unsafe fn bit_not(num: *mut BigNum, gc: &mut GarbageCollector) -> *mut BigNum {
        unimplemented!()
    }
