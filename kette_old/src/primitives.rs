use crate::{
    ...
    object::{
        Array, ByteArray, Object, ObjectRef, ObjectType, Slot, SpecialObjects,
    },
};
...
impl Context {
    ...
    fn parse_quotation(&mut self) {
        ...
        let parser = unsafe { self.parser.as_ptr_unchecked() as *mut Parser };
        let res = unsafe {
            (*parser).parse_until(self, ObjectRef::from_bytearray_ptr(end_word))
        };
    ...
    fn create_new_instance(&mut self) {
        ...
        if unsafe { (*map_ptr).header.map() }
            != self.gc.specials.map_map.as_map_ptr()
        {
            self.push(SpecialObjects::get_false());
            return;
        }
        ...
        unsafe {
            ...
            for i in 0..slot_count {
                ...
                if (*slot).kind == SpecialObjects::get_slot_kind_data() {
                    data_slots
                        .push((i, (*slot).index.as_int_unchecked() as usize));
                }
            }
    ...
    fn parse_syntax(&mut self) {
        ...
        self.namestack_push_or_replace(
            name_obj,
    ...
    fn resize_array(&mut self) {
        ...
        unsafe {
            ...
            std::ptr::copy_nonoverlapping(
                src,
                dst,
                copy_size * std::mem::size_of::<ObjectRef>(),
            );
        }
    ...
    fn array_copy(&mut self) {
        ...
        unsafe {
            ...
            std::ptr::copy(
                src_ptr,
                dst_ptr,
                size * std::mem::size_of::<ObjectRef>(),
            );
        }
    }
    ...
    fn set_alien_u8(&mut self) {
        ...
        unsafe {
            ...
            let value_ptr =
                (value as *mut u8).add(std::mem::size_of::<ByteArray>());
    ...
    fn alien_u8(&mut self) {
        ...
        unsafe {
            ...
            let src_ptr = (src as *mut u8)
                ...
                .add(offset);
            let dst_ptr =
                (new_array as *mut u8).add(std::mem::size_of::<ByteArray>());
    ...
    fn fixnum_to_alien_u8(&mut self) {
        ...
        unsafe {
            let ptr =
                (bytearray as *mut u8).add(std::mem::size_of::<ByteArray>());
            *ptr = num;
        }
...
mod tests {
    ...
    fn test_resize_array() {
        ...
        unsafe {
            ...
            assert_eq!(
                (*expanded).get_element(0),
                Some(ObjectRef::from_int(1))
            );
            assert_eq!(
                (*expanded).get_element(1),
                Some(ObjectRef::from_int(2))
            );
        }
    }
    ...
    fn test_create_map_and_instance() {
        ...
        unsafe {
            let name_name =
                ObjectRef::from_bytearray_ptr(ctx.gc.allocate_string("name"));
            ...
            let age_name =
                ObjectRef::from_bytearray_ptr(ctx.gc.allocate_string("age"));
            ...
            let meta_name =
                ObjectRef::from_bytearray_ptr(ctx.gc.allocate_string("meta"));
            ...
            (*slots_array)
                .set_element(0, ObjectRef::from_ptr(name_slot as *mut _));
            (*slots_array)
                .set_element(1, ObjectRef::from_ptr(age_slot as *mut _));
            (*slots_array)
                .set_element(2, ObjectRef::from_ptr(meta_slot as *mut _));
    ...
    fn test_map_with_only_data_slots() {
        ...
        unsafe {
            let slot_name1 =
                ObjectRef::from_bytearray_ptr(ctx.gc.allocate_string("field1"));
            ...
            let slot_name2 =
                ObjectRef::from_bytearray_ptr(ctx.gc.allocate_string("field2"));
    ...
    fn test_compile_string_nested_quotations() {
        ...
        unsafe {
            ...
            assert_eq!(
                (*nested_array).get_element(0).unwrap().as_int(),
                Some(3)
            );
    ...
    fn test_compile_string_with_primitives() {
        ...
        unsafe {
            ...
            assert_eq!(
                (*word_ptr).body.as_int(),
                Some(Context::fixnum_add as i64)
            );
    ...
    fn test_compile_string_with_mixed_primitives() {
        ...
        unsafe {
            ...
            ctx.namestack_push_or_replace(
                add_name.into(),
                ObjectRef::from_word_ptr(add_word),
            );
            ctx.namestack_push_or_replace(
                dup_name.into(),
                ObjectRef::from_word_ptr(dup_word),
            );
            ...
            let text =
                ctx.gc.allocate_string("21 prim-dup prim-add parser-word");
            let result = ctx.compile_string(text.into());
            let quotation = result.as_quotation_ptr().unwrap();
            let array = (*quotation).body.as_array_ptr().unwrap();
    ...
    fn test_syntax_definition() {
        ...
        unsafe {
            let syntax_def = ctx.gc.allocate_string(
            ...
            ctx.namestack_push_or_replace(
                ObjectRef::from_bytearray_ptr(value_name),
                test_value,
            );
