...
mod tests {
    ...
    unsafe fn create_test_map(
        ...
        let object_size_bytes = std::mem::size_of::<Object>()
            + (object_slots * std::mem::size_of::<ObjectRef>());
    ...
    fn test_object_slots() {
        unsafe {
            ...
            (*parent_map)
                .set_slot(ObjectRef::from_ptr(x_slot as *mut Object), 0);
            ...
            let parent_slot =
                create_test_slot(b"parent", ObjectRef::from_int(2));
            (*parent_slot).ty = ObjectRef::from_ptr(parent_obj);

            (*child_map)
                .set_slot(ObjectRef::from_ptr(y_slot as *mut Object), 0);
            (*child_map)
                .set_slot(ObjectRef::from_ptr(parent_slot as *mut Object), 1);
            ...
            assert_eq!(
                x_value,
                Some(ObjectRef::from_int(100)),
                "x value mismatch"
            );

            (*child_obj)
                .set_slot_value_by_name(&*y_lookup, ObjectRef::from_int(300));
