...
use crate::{
    bignum::BigNum,
    object::{
        Array, BoxObject, ByteArray, Float, Map, Object, ObjectHeader,
        ObjectRef, Quotation, Slot, SpecialObjects, Word,
    },
};
...
impl Chunk {
    ...
    unsafe fn allocate(&mut self, size: usize) -> Option<*mut u8> {
        ...
        let aligned_size = (total_size + MIN_OBJECT_ALIGNMENT - 1)
            & !(MIN_OBJECT_ALIGNMENT - 1);
        ...
        F: FnMut(*mut ObjectHeader),
    {
        ...
        while current < self.current {
            ...
            let object =
                current.add(ALLOCATION_METADATA_SIZE) as *mut ObjectHeader;
            ...
            if live_count == 0 {
                ...
                if i <= self.current_chunk_idx {
                    self.current_chunk_idx =
                        self.current_chunk_idx.saturating_sub(1);
                }
            } else {
                i += 1;
            }
    ...
    pub unsafe fn deallocate(&mut self, ptr: *mut Object) {
        ...
        let layout = unsafe {
            alloc::Layout::from_size_align_unchecked(
                size.as_int_unchecked() as usize,
                8,
            )
        };
...
impl GarbageCollector {
    ...
    pub unsafe fn allocate(&mut self, map: *mut Map) -> *mut Object {
        ...
        let slots =
            unsafe { (ptr as *mut u8).add(std::mem::size_of::<Object>()) };
        unsafe {
            std::ptr::write_bytes(
                slots,
                0,
                size - std::mem::size_of::<Object>(),
            )
        };
    ...
    pub unsafe fn allocate_array(&mut self, length: usize) -> *mut Array {
        ...
        let elements =
            unsafe { (ptr as *mut u8).add(std::mem::size_of::<Array>()) };
        unsafe {
            std::ptr::write_bytes(
                elements,
                0,
                length * std::mem::size_of::<ObjectRef>(),
            )
        };
    ...
    pub unsafe fn allocate_bytearray(
        &mut self,
        length: usize,
    ) -> *mut ByteArray {
        ...
        let elements =
            unsafe { (ptr as *mut u8).add(std::mem::size_of::<ByteArray>()) };
    ...
    pub unsafe fn allocate_slot(
        ...
        let slot =
            unsafe { self.allocate(self.specials.get_slot_map()) as *mut Slot };
    ...
    pub unsafe fn allocate_quotation(
        &mut self,
        size: Option<usize>,
    ) -> *mut Quotation {
        let ptr =
            self.allocate(self.specials.get_quotation_map()) as *mut Quotation;
    ...
    unsafe fn mark(&mut self) {
        ...
        while let Some(obj) = work_list.pop() {
            ...
            let slot_count =
                unsafe { map.slot_count.as_int_unchecked() as usize };
            for i in 0..slot_count {
                if let Some(slot) = (*map).get_slot(i) {
                    ...
                    if kind == SpecialObjects::get_slot_kind_data() {
                        let idx = unsafe {
                            (*slot).index.as_int_unchecked() as usize
                        };
    ...
    unsafe fn mark_object(
        &self,
        obj: ObjectRef,
        work_list: &mut Vec<ObjectRef>,
    ) {
        ...
        if !header.is_marked() {
            ...
            if obj.as_bignum_ptr().is_some()
                || map_ptr == self.specials.bignum_map.as_map_ptr()
            {
                return;
            }
    ...
    unsafe fn deep_clone_with_cache(
        ...
        if let Some(array) = obj.as_array_ptr() {
            ...
            if !(*array).size.is_int() {
                (*new_array).size = self.deep_clone_with_cache(
                    (*array).size,
                    cache,
                    current_level + 1,
                    levels,
            ...
            for i in 0..size {
                ...
                let cloned_elem = self.deep_clone_with_cache(
                    elem,
                    cache,
                    current_level + 1,
                    levels,
        ...
        if let Some(bytearray) = obj.as_bytearray_ptr() {
            ...
            let src =
                (bytearray as *const u8).add(std::mem::size_of::<ByteArray>());
            let dst = (new_bytearray as *mut u8)
        ...
        for i in 0..slot_count {
            if let Some(slot_ref) = map.get_slot(i) {
                ...
                if (*slot).is_data_slot() {
                    ...
                    let cloned_value = self.deep_clone_with_cache(
                        value,
                        cache,
                        current_level + 1,
                        levels,
...
impl GarbageCollector {
    pub fn init_special_objects(&mut self) {
        unsafe {
            ...
            let bytearray_map_name = ObjectRef::from_bytearray_ptr(
                self.allocate_string("ByteArray"),
            );
            ...
            let array_map_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("Array"));
            ...
            (*(*map_map).slots.as_array_ptr().unwrap()).header =
                ObjectHeader::new(array_map);

            let slot_map_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("Slot"));
            ...
            let name_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("name"));
            let object_size_name = ObjectRef::from_bytearray_ptr(
                self.allocate_string("object_size"),
            );
            let slot_count_name = ObjectRef::from_bytearray_ptr(
                self.allocate_string("slot_count"),
            );
            let slots_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("slots"));
            let default_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("default"));
            let size_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("size"));

            let slot_type_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("type"));
            let slot_index_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("index"));
            let slot_kind_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("kind"));
            let slot_guard_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("guard"));
            ...
            (*map_slots).set_element(
                0,
                ObjectRef::from_ptr(map_name_slot as *mut Object),
            );
            (*map_slots).set_element(
                1,
                ObjectRef::from_ptr(map_object_size_slot as *mut Object),
            );
            (*map_slots).set_element(
                2,
                ObjectRef::from_ptr(map_slot_count_slot as *mut Object),
            );
            (*map_slots).set_element(
                3,
                ObjectRef::from_ptr(map_slots_slot as *mut Object),
            );
            (*map_slots).set_element(
                4,
                ObjectRef::from_ptr(map_default_slot as *mut Object),
            );
            ...
            (*array_slots).set_element(
                0,
                ObjectRef::from_ptr(array_size_slot as *mut Object),
            );
            let bytearray_slots =
                (*bytearray_map).slots.as_array_ptr().unwrap();
            (*bytearray_slots).set_element(
                0,
                ObjectRef::from_ptr(bytearray_size_slot as *mut Object),
            );
            ...
            (*slot_slots).set_element(
                0,
                ObjectRef::from_ptr(slot_name_slot as *mut Object),
            );
            (*slot_slots).set_element(
                1,
                ObjectRef::from_ptr(slot_type_slot as *mut Object),
            );
            (*slot_slots).set_element(
                2,
                ObjectRef::from_ptr(slot_index_slot as *mut Object),
            );
            (*slot_slots).set_element(
                3,
                ObjectRef::from_ptr(slot_kind_slot as *mut Object),
            );
            (*slot_slots).set_element(
                4,
                ObjectRef::from_ptr(slot_guard_slot as *mut Object),
            );

            let slot_map_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("True"));
            ...
            let float_map_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("Float"));
            ...
            let bignum_map_name = ObjectRef::from_bytearray_ptr(
                self.allocate_string("ByteArray"),
            );
            ...
            let quotation_map_name = ObjectRef::from_bytearray_ptr(
                self.allocate_string("Quotation"),
            );
            ...
            let body_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("body"));
            let stack_effect_name = ObjectRef::from_bytearray_ptr(
                self.allocate_string("stack-effect"),
            );
            let compiled_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("compiled"));
            ...
            let quotation_slots =
                (*quotation_map).slots.as_array_ptr().unwrap();
            (*quotation_slots).set_element(
                0,
                ObjectRef::from_ptr(quotation_body_slot as *mut Object),
            );
            (*quotation_slots).set_element(
                1,
            ...
            let word_map_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("Word"));
            ...
            let word_name_slot_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("name"));
            let word_body_slot_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("body"));
            let word_stack_effect_slot_name = ObjectRef::from_bytearray_ptr(
                self.allocate_string("stack-effect"),
            );
            let word_flags_slot_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("flags"));
            ...
            (*word_slots).set_element(
                0,
                ObjectRef::from_ptr(word_name_slot as *mut Object),
            );
            (*word_slots).set_element(
                1,
                ObjectRef::from_ptr(word_body_slot as *mut Object),
            );
            (*word_slots).set_element(
                2,
            ...
            (*word_slots).set_element(
                3,
                ObjectRef::from_ptr(word_flags_slot as *mut Object),
            );

            let box_map_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("Box"));
            ...
            // Add box value slot
            let value_name =
                ObjectRef::from_bytearray_ptr(self.allocate_string("value"));
            ...
            (*box_slots)
                .set_element(0, ObjectRef::from_ptr(value_slot as *mut Object));
...
mod tests {
    ...
    fn test_chunk_scanning() {
        unsafe {
            ...
            let obj1 =
                chunk.allocate(32).expect("First allocation should succeed")
                    as *mut ObjectHeader;
            let obj2 = chunk
                .allocate(32)
    ...
    fn test_gc_large_object_allocation() {
        ...
        unsafe {
            ...
            let large_array = gc
                .allocate_array(large_size / std::mem::size_of::<ObjectRef>());
    ...
    fn test_clone_simple_object() {
        ...
        unsafe {
            let map_name =
                ObjectRef::from_bytearray_ptr(gc.allocate_string("Test Type"));
            let slot_name =
                ObjectRef::from_bytearray_ptr(gc.allocate_string("value"));
    ...
    fn test_clone_array() {
        ...
        unsafe {
            ...
            assert_eq!(
                (*cloned_array).get_element(0),
                Some(ObjectRef::from_int(1))
            );
            assert_eq!(
                (*cloned_array).get_element(1),
                Some(ObjectRef::from_int(2))
            );
            assert_eq!(
                (*cloned_array).get_element(2),
    ...
    fn test_clone_bytearray() {
        ...
        unsafe {
            ...
            let cloned =
                gc.deep_clone(ObjectRef::from_bytearray_ptr(original), 0);
