...
pub enum Instruction {
    LoadConstant { idx: u16 },
    LoadSmi { value: i32 },
    Return,
    LocalReturn,
    CreateObject { map_idx: u16, values_reg: u16 },
    CreateBlock { block_idx: u16 },
    Send { message_idx: u16, reg: u16, argc: u8, feedback_idx: u16 },
    LoadLocal { reg: u16 },
    StoreLocal { reg: u16 },
    LoadStack { offset: u32 },
    StoreStack { offset: u32 },
    LoadTemp { array_idx: u16, idx: u16 },
    StoreTemp { array_idx: u16, idx: u16 },
    LoadAssoc { idx: u16 },
    StoreAssoc { idx: u16 },
    Mov { dst: u16, src: u16 },
    MovToStack { offset: u32, src: u16 },
    MovFromStack { dst: u16, offset: u32 },
    MovToTemp { array_idx: u16, idx: u16, src: u16 },
    MovFromTemp { dst: u16, array_idx: u16, idx: u16 },
    MovToAssoc { idx: u16, src: u16 },
    MovFromAssoc { dst: u16, idx: u16 },
    Jump { offset: i16 },
...
impl fmt::Display for Instruction {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::CreateObject { map_idx, values_reg } => {
                write!(f, "CreateObject #{map_idx} r{values_reg}")
            }
            Self::CreateBlock { block_idx } => write!(f, "CreateBlock #{block_idx}"),
            Self::Send { message_idx, reg, argc, feedback_idx } => {
            ...
            Self::LoadTemp { array_idx, idx } => write!(f, "LoadTemp #{array_idx}[{idx}]"),
            Self::StoreTemp { array_idx, idx } => write!(f, "StoreTemp #{array_idx}[{idx}]"),
            ...
            Self::MovToStack { offset, src } => write!(f, "MovToStack @{offset}, r{src}"),
            Self::MovFromStack { dst, offset } => write!(f, "MovFromStack r{dst}, @{offset}"),
            Self::MovToTemp { array_idx, idx, src } => {
                write!(f, "MovToTemp #{array_idx}[{idx}], r{src}")
            }
            Self::MovFromTemp { dst, array_idx, idx } => {
                write!(f, "MovFromTemp r{dst}, #{array_idx}[{idx}]")
            }
            Self::MovToAssoc { idx, src } => write!(f, "MovToAssoc #{idx}, r{src}"),
            Self::MovFromAssoc { dst, idx } => write!(f, "MovFromAssoc r{dst}, #{idx}"),
